...
# Defaults: clew  christian-luger-at/obsidian-clew

set -euo pipefail

PLUGIN_ID="${1:-clew}"
GITHUB_REPO="${2:-christian-luger-at/obsidian-clew}"
...
# Not being in the store yet is normal before the community-plugin PR is merged —
# fall back to the GitHub numbers instead of bailing out.
STORE_TMP=$(mktemp)
if [[ -z "$PLUGIN_DATA" ]]; then
  echo "Note: plugin '$PLUGIN_ID' is not in the Obsidian store stats yet; showing GitHub downloads only." >&2
else
