...
**Not done in this spike** (deferred to an actual implementation, gated on this decision being accepted): the caching layer, incremental re-embedding on note change, offloading inference to a Web Worker (the ~2.5ms/note figure was measured single-threaded/synchronous - fine for a background job, but should not block the main thread in the real integration, same reasoning `layoutRunner.ts`'s ForceAtlas2 Worker already follows), the new `nodeGroups.ts` criterion type ("semantic similarity to a chosen note" or a standalone clustering pass - community detection's own `detectCommunities()` in `stagnation.ts` is the more likely integration point than a per-pair criterion, since "cluster" is inherently a whole-graph computation, not a single note's property) or its Color & size UI, and bundling/serving the ONNX Runtime Web `.wasm` binary itself from inside the plugin folder (currently defaults to fetching from a CDN, which the plugin's `manifest.json`/Community-store review would very likely need changed to a locally-shipped file for privacy/offline reasons - not investigated in depth here). GitHub issue #16 stays open for that follow-up implementation work.
