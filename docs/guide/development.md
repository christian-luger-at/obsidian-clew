...
1. Builds `.screenshot-vault/` - the same demo vault `npm run gen-test-vault` generates, plus the built plugin and a couple of pre-configured filters/groups so the Filter and Color & size panels aren't shown empty.
...
4. Opens the graph view wide, waits for Force layout to settle, and captures each motif (graph overview, layout picker, Filter panel, Color & size panel, Appearance panel, node hover) in light and dark.
