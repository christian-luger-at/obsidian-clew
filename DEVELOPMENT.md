...
- **Graph-Analytics erweitern** (GitHub backlog item 5 - Bridging/
  ...
  - **Community**: create a group with "Community is [1]" (1-based in the
    ...
    criterion via "+ Add" → "Community", then pick different notes in the
    "Pick a note in it…" text field a few times - the group's color should
    never change on its own at any point (see the removed-feature note
    right below) - only its own color picker changes it. Same check for
    `semanticCluster`.

    Originally shipped as "Community-Färbung mit fester Palette" (GitHub
    backlog item 5) - a Color & size group's `color` auto-synced to a fixed
    palette entry (`communityColor(communityId)`, now removed -
    `DEFAULT_GROUP_COLORS[communityId % length]`) every time a
    `community`/`semanticCluster` criterion's id was picked or changed
    (`CriteriaEditorContext.onCommunityColorSync`, also now removed), so
    "Community 1 is red" stayed consistent across sessions without a
    separate coloring UI. Removed entirely after two rounds of user
    feedback:
    1. "Wenn ich +add drücke und Community wähle, dann ist die Farbe rot" -
       `openAddCriterionMenu()` called the same sync *at add time too*,
       right after `blankCriterion()` created a fresh criterion still at
       its default `communityId`/`clusterId: 0` - so every newly-added
       criterion immediately overwrote the group's color with
       `communityColor(0)` (literally red), clobbering whatever color the
       group already had before any real community/cluster had even been
       picked. First fix: removed just the add-time sync call, kept the
       pick-time one (in `renderCriterionEditRow()`'s `onSelect` handlers).
    2. Still not enough - "Wenn ich dann einen Wert in das Textfeld
       schreibe, dann wechselt die Farbe nochmals", and explicitly, when
       asked, that the resulting color was itself "falsch/unpassend"
       (wrong/mismatched) - i.e. the *pick-time* sync (color follows
       whichever community/cluster you pick) wasn't wanted either. Asked
       directly: keep auto-color (fix the mismatch) or make color fully
       manual? User: "Farbe soll manuell bleiben, nie automatisch ändern".
       Removed the whole mechanism - `communityColor()`,
       `onCommunityColorSync` (interface field, `groupCriteriaContext()`'s
       provider, both `onSelect` call sites) - a Color & size group's color
       is now purely and only what its own color picker sets, same as
       every other criterion type always worked; `communityColor()`'s own
       tests removed along with it (dead code, nothing left to call it).
...
User report: "Wenn ich einen Semantic Cluster als Filter oder Color wähle, dann kann ich den Cluster nicht wählen. Es steht nur 'Computing embeddings....'" - stuck forever, not just slow. Root cause (`refreshSemanticClusters()`): the initial `if (this.colorAndSizePanelEl.isShown()) this.renderColorAndSizePanel();` right after setting `semanticClusteringStatus = 'loading'` correctly painted the "Computing embeddings…" message, but nothing re-rendered either panel once the method actually *finished* - `semanticClusteringStatus` flipped to `'ready'`/`'error'` in memory same as always, but the DOM stayed frozen on whatever that loading-state render last produced. `refreshCriteriaContent()` (this method's only caller) went on to call `repaintNodeGroups()`/`applyFilter()` afterward, neither of which touches either panel's own DOM - a panel only otherwise rebuilds on an explicit user action (add/edit/delete a criterion, open/close the panel), and "an unrelated async task finished in the background" was never one of those triggers. Fixed by re-rendering whichever of Color & size/Filter is currently open (both, in a `finally`, not just one - a `semanticCluster` criterion can live in either, and there's no cheap way to know from inside `refreshSemanticClusters()` which panel the criterion that triggered this specific call actually belongs to). Manual QA: add a `semanticCluster` criterion in Color & size (or Filter) on a vault with no cached embeddings yet (first use this session, or after a plugin reload) - the "Computing embeddings…" message should switch to the note-picker on its own within a few seconds (longer on the very first model download), with no need to close/reopen the panel or touch anything else to "unstick" it. Same check with the Filter panel open instead. Trigger it from Color & size while Filter is *also* open (or vice versa) - both panels' relevant criterion rows should update, not just the one that was actually edited.
...
## Code-fence embed (GitHub issue #4)

GitHub issue #4, "Code-Fence Embed": a ` ```clew-graph ` code block inside any note renders a small, inline ego-graph, centered on one note (`node:`, resolved the same way a `[[wikilink]]` is - bare basename, a relative path, or a full vault path) out to a chosen number of hops (`hops:`, 1-3, defaults to 1 if omitted). Scoped deliberately small for this first version, per the issue's own spec: no filter UI at all, just these parameters.

```
​```clew-graph
node: World War II Era
hops: 2
width: 600px
height: 40vh
refresh: true
...
- **`src/graph/embedConfig.ts`** - `parseEmbedConfig()`, a small `key: value` line parser (not a full YAML parse - there are only ever a handful of fields), pure and Obsidian-free so it's unit-testable without dragging in GraphPane's own Sigma/WebGL dependency chain (`embedConfig.test.ts` - the module lives apart from `graphEmbed.ts` for exactly this reason, same split as `egoGraph.ts`/`nodeGroups.ts` from `graphPane.ts`). `hops` outside 1-3 is clamped, not rejected; `width`/`height` accept a CSS length (`600px`, `50%`, a bare number treated as px) and fall back to `null` (not set) rather than erroring on anything that doesn't parse as one; `refresh` is a loose boolean (`true`/`yes`/`1`/`on`, case-insensitive).
- **`src/graph/graphEmbed.ts`** - `registerGraphEmbed()` (called once from `main.ts`'s `onload()`) registers the code-fence processor; `ClewGraphEmbed` (a `MarkdownRenderChild`, not a plain callback) owns one embedded `GraphPane`'s lifecycle so `onunload()` gets called and can kill the Sigma renderer - reading mode re-runs the processor on every edit/scroll-into-view, and without this a live Sigma instance would leak on each one. Resolves `node:` via `getFirstLinkpathDest()`; a missing `node:` or an unresolvable note name renders a plain red error message in the block's place instead of throwing. Sets `frameEl.style.width`/`height` inline only for whichever of the two the fence actually gave (see `styles.css` below for what happens to the other). When `refresh: true`, adds a small floating icon button (top-right of the frame) that just re-runs the same `setFiles()` + `applyFocus()` pair used on first load - a manual way to pick up vault changes, since (deliberately, for a "small fixed illustration") the embed doesn't wire up any of the live vault-change listeners `StandaloneGraphView` does. Also owns a `ResizeObserver` on the embed's frame (Sigma doesn't track its own container's size - see `GraphPane.handleResize()`'s docstring - and a plain div inside rendered markdown gets none of the workspace resize events the standalone view relies on).
- **`GraphPane`** gained a constructor `options.embedded` flag rather than a second code path: the toolbar/every dialog panel/legend/timeline bar are still built exactly as normal (avoids making ~15 fields nullable throughout the rest of this 6000-line file for no real benefit) - `embedded` just adds a `.clew-graph-embed` class (hides all of that via CSS) and stops the pane from ever claiming `GraphPane.active`, so clicking inside a small inline ego-graph can't hijack the main "Find path" command's target. `applyFocus()` (already existed, backing the Focus panel) was made public - the embed calls it directly right after `setFiles()`, with no Focus panel of its own to drive it.
- **User feedback, before the first version shipped**: "Es dürfen keine Filter, Colors oder gespeicherte Positionen übernommen werden" - an embed shouldn't be a miniature of whatever the user's real graph view happens to be showing. Three new private helpers (`effectiveNodeGroups()`/`effectiveFilterPresets()`/`effectivePinnedPositions()`) return an empty list/`{}` for an embedded pane regardless of what's actually saved, and every render-affecting read of `plugin.settings.nodeGroups`/`filterPresets`/`pinnedPositions` was routed through them (`paintVisualEncoding()`'s group coloring, `currentFilterMatches()`, `setFiles()`'s/`setForceLayout()`'s pinned-position reads). Node dragging (`setupNodeDragging()`, which persists a dropped node's position into `plugin.settings.pinnedPositions`) is skipped entirely for an embedded pane rather than just gated at read time - the requirement is "don't carry over," which also means "don't let an embed write a pin into the user's real settings" if someone drags a node inside one.
- **A real bug found via the manual QA below, not by inspection**: `allCriteriaOwners()` (feeds `refreshCriteriaContent()`'s "does any active criterion need note-content search/semantic clustering" check) still read `plugin.settings.nodeGroups`/`filterPresets` directly. On a vault with an unrelated real `text` filter enabled, an embedded pane reported "needs content search" for a filter it would never actually apply, kicked off `refreshCriteriaContent()`'s async content-cache read, and that method's own trailing `applyFilter()` call - finding `effectiveFilterPresets()` correctly empty ("no filter enabled") - called `clearFocus()`, silently wiping the `applyFocus()` the embed had just set up a moment earlier. Net effect: every embed briefly showed its correct ego-graph, then a moment later expanded to render the *entire* vault, with nothing in `graphEmbed.ts` itself doing anything wrong - only reproducible with a real filter/group present in the vault's actual settings, which is exactly why the QA vault below deliberately seeds one. Fixed by routing `allCriteriaOwners()` through the same two `effective*()` helpers.
- **`styles.css`** - `.clew-graph-embed-frame` defaults to full width and `aspect-ratio: 16 / 9` (per the spec: "Wenn keine Breite, dann 100% nutzen; 16:9 Format verwenden, wenn kein Parameter für Höhe und Breite gesetzt") - `aspect-ratio` only takes effect while height is still `auto`, so an explicit inline `height` (set by `graphEmbed.ts` only when the fence's own `height:` was given) naturally overrides it without this stylesheet fighting itself. Verified directly that this default needs `.clew-graph-embed-frame.clew-graph-view` (two classes, not one) to actually win: `GraphPane`'s constructor stamps the same element with `.clew-graph-view` too, which has equal CSS specificity to a bare `.clew-graph-embed-frame` and, being declared later in the file, silently overrode it in the first version of this rule regardless of which sizing approach was used - real repro was every embed rendering as an invisible ~2px sliver, root-caused by inspecting the frame's actual `getBoundingClientRect()` in a live Obsidian instance rather than guessing.

Manual QA (real Obsidian, `.embed-qa-vault` built the same way `scripts/screenshots.mjs` builds its own demo vault, plus a **real, enabled** filter/node-group/pinned-position in the vault's own `data.json` - not just an empty/disabled one, which is exactly what the `allCriteriaOwners()` bug above needed to surface - and a throwaway note with several ` ```clew-graph ` blocks; script not committed, same "verify, don't guess" approach as the semantic-clustering spike above):

- A default (no `width`/`height`) block rendered at exactly 592x333px (100% of the note's content width, 16:9) showing only `World War II Era`'s 1-hop neighborhood (~15 notes, not the vault's 296) - uniformly violet, normal node size, confirming none of the seeded red/3x-oversized node group leaked in.
- A `width: 400px` / `height: 250px` block rendered at exactly that pixel size, same correctly-restricted ego-graph.
- A `refresh: true` block showed the floating refresh icon (absent on the other two); clicking it re-rendered the same ego-graph without visibly disturbing the rest of the note.
- A block missing `node:` rendered `clew-graph: missing "node: <note name>".` in red, no crash; a block with an unresolvable `node: Does Not Exist` rendered `clew-graph: note "Does Not Exist" not found.` in red, no crash.

### Visual polish round (user feedback: "die Darstellung [...] ist nicht schön")

Five changes, all embed-only (the real graph view's own look is untouched):

- **Radial layout instead of Force.** `GraphPane.setRadialFocus()` (a thin public wrapper around the existing private `setRadialLayout()`, GitHub backlog item 3's own layout) is called before `applyFocus()` - clean concentric rings around the center note read far better at a small embed's scale than Force's randomly-scattered settle, and it's also *how* the next point gets its ring "for free": `computeRadialLayout()` already places the focus node at the origin, and `activateLayoutMode('radial')` already repaints the "singled-out" ring (`paintVisualEncoding()`'s `singledOutNodePath`, the same mechanism Radial/Find-path already use) around whatever `radialFocusNode` is.
- **Center node highlighted.** `applyFocus()` gained an options param, `{ highlightCenter, fitCamera }`, both defaulting `false` so the real Focus panel's own behavior is unchanged - the embed passes both `true`. `highlightCenter` rings the center node (same accent/gap-color pair as every other "singled-out" ring in this file, Find-path's green - not a new color) at `FOCUS_CENTER_SIZE_MULTIPLIER` (1.6x) its normal size, and force-shows its label (`forceLabel: true`) regardless of the density/threshold settings below, so it reads as "this is what the embed is about" even in a dense neighborhood.
- **Label decluttering.** `EMBED_LABEL_SIZE_THRESHOLD_FLOOR` (10) - `setFiles()` now passes `Math.max(appearance.labelSizeThreshold, EMBED_LABEL_SIZE_THRESHOLD_FLOOR)` to `createRenderer()` for an embedded pane, a floor not an override (a user who's already set an even higher threshold for their real graph view keeps it). Combined with the center node's own `forceLabel: true` above, the result is "only the note this embed is about gets a permanent label, everything else needs a large-enough on-screen size (or a hover) to earn one" - directly targets the overlapping-label mess a small canvas otherwise produces.
- **A caption above the frame.** `graphEmbed.ts` creates a `.clew-graph-embed-caption` div (`"<note name> · N hops"`) as a sibling before the frame - gives context without opening the code fence.
- **A softer frame.** `.clew-graph-embed-frame` gained `box-shadow: var(--shadow-s)` alongside its existing hairline border, same "hairline + shadow lifts it off the page" idea the docs site's own `.doc-shot` screenshots already use (`docs/.vitepress/theme/custom.css`), instead of reading as a flat, hard-edged box.

The camera-fit needed its own fix to make Radial actually work for a *restricted* (ego-set-only) view: `resetCameraAndRefresh()`/`fittedBBox()` both gained an optional node-set parameter (`fitTo`/`only`) - without it, fitting to *every* node's position (the default, and what every other caller still gets) would frame however far Radial actually laid out the whole graph, most of which Focus has just hidden; the embed's own ego-graph would end up a speck in a mostly-empty frame. `applyFocus()`'s `fitCamera: true` passes its own `egoSet` here. `setRadialFocus()` itself skips its normal (whole-graph) camera fit (`setRadialLayout(focusPath, fitCamera: false)`) so the two don't run as competing camera animations a frame apart - `applyFocus()`'s own fit is the one that actually lands.

Manual QA (real Obsidian, both themes): a 2-hop and a 1-hop embed of `World War II Era` both rendered as clean concentric rings, center node visibly larger with a ring and a permanent label, every other label hidden (no more overlapping smear), a caption above each frame, a visible soft shadow around each frame in both light and dark. Zooming into the center node directly confirmed the ring renders correctly (a distinct outer band, not just a bigger flat dot).
