...
User-reported: `TypeError: Cannot read properties of undefined (reading 'create')` in the console, deep inside ONNX Runtime Web, the first time a `semanticCluster` criterion tried to load the model
- Root cause: transformers.js defaults to ONNX Runtime Web's multi-threaded WASM backend, which needs `SharedArrayBuffer`/`crossOriginIsolated` (the browser-standard cross-origin-isolation gate) to spin up its worker thread pool - real sites get that by serving the right `Cross-Origin-Opener-Policy`/`Cross-Origin-Embedder-Policy` headers; Obsidian's Electron renderer (`app://` origin) doesn't set those, so `crossOriginIsolated` is `false` there and the threaded backend's own init silently produces a partially-formed object instead of a clean error - the very next call into it throws this.
- Fix (`embeddingModel.ts`): `env.backends.onnx.wasm.numThreads = 1` (set once, at module load, before the first `pipeline()` call - ONNX Runtime Web's own documented fallback for exactly this, sidesteps the SharedArrayBuffer path entirely) plus an explicit `device: 'wasm'` in the `pipeline()` call itself (transformers.js's own `'auto'` default picks WebGPU whenever the environment merely *reports* the API as present, which Electron's renderer does even where the rest of the stack isn't as usable as a real browser's - `'wasm'` is the exact path the spike measured).
- Verified in a real (non-Obsidian) browser context deliberately chosen to match the failure condition: a plain `python3 -m http.server` page (no COOP/COEP headers, so `crossOriginIsolated: false` there too, same as `app://`) running the actual production `embeddingModel.ts`, bundled via the same `esbuild --platform=browser` path the real plugin uses - confirmed `crossOriginIsolated` reads `false` and the model still loads and returns a real 384-dimension vector, where it previously would have thrown. Not verified inside Obsidian itself in this session.
