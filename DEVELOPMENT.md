...
- **`src/graph/embedConfig.ts`** - `parseEmbedConfig()`, a small `key: value` line parser (not a full YAML parse - there are only ever two fields), pure and Obsidian-free so it's unit-testable without dragging in GraphPane's own Sigma/WebGL dependency chain (`embedConfig.test.ts` - the module lives apart from `graphEmbed.ts` for exactly this reason, same split as `egoGraph.ts`/`nodeGroups.ts` from `graphPane.ts`). `hops` outside 1-3 is clamped, not rejected.
- **`src/graph/graphEmbed.ts`** - `registerGraphEmbed()` (called once from `main.ts`'s `onload()`) registers the code-fence processor; `ClewGraphEmbed` (a `MarkdownRenderChild`, not a plain callback) owns one embedded `GraphPane`'s lifecycle so `onunload()` gets called and can kill the Sigma renderer - reading mode re-runs the processor on every edit/scroll-into-view, and without this a live Sigma instance would leak on each one. Resolves `node:` via `getFirstLinkpathDest()`; a missing `node:` or an unresolvable note name renders a plain red error message in the block's place instead of throwing. Also owns a `ResizeObserver` on the embed's frame (Sigma doesn't track its own container's size - see `GraphPane.handleResize()`'s docstring - and a plain div inside rendered markdown gets none of the workspace resize events the standalone view relies on).
- **`GraphPane`** gained a constructor `options.embedded` flag rather than a second code path: the toolbar/every dialog panel/legend/timeline bar are still built exactly as normal (avoids making ~15 fields nullable throughout the rest of this 6000-line file for no real benefit) - `embedded` just adds a `.clew-graph-embed` class (hides all of that via CSS, `styles.css`) and stops the pane from ever claiming `GraphPane.active`, so clicking inside a small inline ego-graph can't hijack the main "Find path" command's target. `applyFocus()` (already existed, backing the Focus panel) was made public - the embed calls it directly right after `setFiles()`, with no Focus panel of its own to drive it.
- **`styles.css`** - `.clew-graph-embed-frame` gives the embed a fixed 400px height (`.clew-graph-view` below it is `height: 100%`, which collapses to 0 inside a markdown-rendered note's auto-height container without this). Verified directly that the height rule needs `.clew-graph-embed-frame.clew-graph-view` (two classes, not one) to actually win: GraphPane's constructor stamps the same element with `.clew-graph-view` too, which has equal CSS specificity to a bare `.clew-graph-embed-frame` and, being declared later in the file, silently overrode it in the first version of this rule - real repro was every embed rendering as an invisible ~2px sliver, root-caused by inspecting the frame's actual `getBoundingClientRect()` in a live Obsidian instance rather than guessing.

Manual QA (real Obsidian, `.embed-qa-vault` built the same way `scripts/screenshots.mjs` builds its own demo vault, plus a throwaway note with three ` ```clew-graph ` blocks - script not committed, same "verify, don't guess" approach as the semantic-clustering spike above): a valid `node: World War II Era` / `hops: 2` block rendered a 592x400 frame showing exactly that note's 2-hop neighborhood, no toolbar/legend visible, Force-settled and legible; a block missing `node:` rendered `clew-graph: missing "node: <note name>".` in red, no crash; a block with an unresolvable `node: Does Not Exist` rendered `clew-graph: note "Does Not Exist" not found.` in red, no crash.
